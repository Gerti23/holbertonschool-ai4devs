...
1. **Basic mixed severities**  
   Input: 4 valid events in one 60s window, 1 with `ERROR`, latencies `[100, 120, 200, 310]`, endpoints `/a, /a, /b, /c`, `top_k=2`  
   Expected: `total_events=4`, `error_events=1`, `error_rate=0.25`, `p95_latency_ms=310`, `top_endpoints=[(/a,2),(/b,1)]`
...
6. **Single event percentile**  
   Input: one valid event with latency `450`  
   Expected: `p95_latency_ms=450`, `error_rate` depends on severity (`1.0` if `ERROR`, else `0.0`)

7. **Top-K larger than unique endpoints**  
