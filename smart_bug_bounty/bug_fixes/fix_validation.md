...
- **Test Results**: Automated execution blocked (`node: command not found`). Static validation confirms fix logic preserves falsy values.
...
- **Test Results**: Automated execution blocked (`javac: command not found`). Static validation confirms no false-positive character is returned when none is unique.
...
- **Test Results**: Automated execution blocked (`go: command not found`). Static validation confirms floating-point division now preserves decimals.
